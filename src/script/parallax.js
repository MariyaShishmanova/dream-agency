document.addEventListener('DOMContentLoaded', function() {
  ...
  sectionCta.addEventListener('mousemove', function(event) {
    ...
    images.forEach((image, index) => {
      ...
      const offsetX = (image.offsetLeft + (image.offsetWidth / 2) - mouseX) / intensity; // Adjust the division value for the desired parallax effect
      const offsetY = (image.offsetTop + (image.offsetHeight / 2) - mouseY) / intensity; // Adjust the division value for the desired parallax effect
